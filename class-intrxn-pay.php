 ...
 * Author:      intrxn
 * Author URI:  mailto:contact@intrxn.com?subject=Crypto payment checkout by intrXn plugin
 * Version:     1.0
 *
...
function cp_load_intrxn_payment_gateway()
{
    ...
    if (!class_exists('Intrxn_pay')) {
        ...
        class Intrxn_pay extends WC_Payment_Gateway
        {
            ...
            {
                $payment_id = (int)intrxn_Crypto_Helper::get_request_value('id');
            ...
            {
                // Update metadata here so we can process refund from woocommerce
                $payment_id = (int)intrxn_Crypto_Helper::get_request_value('id');
